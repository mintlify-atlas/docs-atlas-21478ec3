---
title: "Live WhatsApp Inbox — Real-Time Chat Interface"
sidebarTitle: "Inbox"
...
To change a conversation's label, the UI sends an AJAX POST to:

```
POST /whatsapp/inbox/<conv_pk>/label/
```

`UpdateConversationLabelView` handles this request and returns `{"status": "ok"}`. Send an empty `label` field to remove the label.
