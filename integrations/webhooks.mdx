...
description: "Configure your Meta App webhook to point at django-meta-whatsapp's built-in endpoint, which auto-processes inbound messages, delivery receipts, and status updates."
---
...
| `processed` | `BooleanField` — whether the payload was processed without error |
| `error` | `TextField` — error message if processing failed |

<Note>
  Webhook logs are written **before** processing, so even if your signal receivers raise an exception, the raw payload is preserved for replay or debugging.
</Note>
